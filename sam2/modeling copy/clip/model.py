    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, pattern=None):
        ...
        if pattern is not None:
            ...
            if "adapter" in pattern:
                # num_experts = 8
                # self.adapter_ffn = EMoE(d_model, int(d_model // (num_experts*2)), num_experts)
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int,
                 heads: int, output_dim: int, cfg: dict, pattern=None):
        super().__init__()
    ...
    def forward_old(self, x: torch.Tensor, cv_emb=None, modality=None, text_inverse=None):
        ...
        if text_inverse is not None:
            text_inverse = text_inverse + cv_emb.squeeze(1) + self.new_positional_embedding.to(x.dtype)
            # add the inverse text features
    ...
    def forward(self, x: torch.Tensor, cv_emb=None, modality=None, text_inverse=None):
        if self.forward_type == 'new':
            return self.forward_new(x, cv_emb, modality, text_inverse)
        else:
            return self.forward_old(x, cv_emb, modality, text_inverse)
...
def resize_pos_embed(posemb, posemb_new, hight, width, cfg=None):
    ...
    if cfg.MODEL.FORWARD == 'new':
        print('Resized position embedding: %s to %s', posemb.shape, posemb_new.shape)
        print('Now is the new forward! The cls token is not included in the position embedding')
    else:
        print('Resized position embedding: %s to %s', posemb.shape, posemb_new.shape)
