...
def evaluate(model, dataloader, device):
    ...
    for images, labels in tqdm(dataloader):
        ...
        if sliding:
            ...
            output = model(images, multimask_output=True) 
            logits = output
...
def main(cfg):
    ...
    for case in cases:
        ...
        num_experts = train_cfg.get('NUM_EXPERTS', 2)
        top_k = train_cfg.get('TOP_K', min(2, num_experts))
        clip_text_dim = 512

        sam2 = build_sam2(model_cf, checkpoint)
        model = LoRA_Sam(sam2, r=train_cfg['RANK'], num_classes=5, num_experts=num_experts, top_k=top_k, text_dim=clip_text_dim)
